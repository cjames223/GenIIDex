                ...
                .then(data1 => {
                    for(let i = 0; i < imgDiv.length; i++) {
                            ...
                            .then(data2 => {
                                imgDiv[i].addEventListener('click', () => {
                                    if(imgDiv[i].id === data2.name) {
                                        ...
                                        height.innerText = data1.height
                                        weight.innerText = data1.weight
                        ...
                        search.addEventListener('search', () => {
                            if(search.value.toLowerCase() === imgDiv[i].id.toLowerCase() || search.value.toLowerCase() === type1[i].innerText.toLowerCase() || search.value.toLowerCase() === type2[i].innerText.toLowerCase()) {
                                imgDiv[i].style.display = 'block'
                            }
                            if(search.value.toLowerCase() !== imgDiv[i].id.toLowerCase() && search.value.toLowerCase() !== type1[i].innerText.toLowerCase() && search.value.toLowerCase() !== type2[i].innerText.toLowerCase()) {
                                imgDiv[i].style.display = ''
